...
public class Hierarchy {
    ...
    public static void main(String[] args) throws IOException {
        ...
        br.close();
        float totalSum = 0;
        List<Edge> edges = g.prim(g.searchIndex(sef));
        for(int i = 0; i<edges.size(); i++)
        {
            totalSum+=edges.get(i).getWeight();
        }
...
class Graph<E> {
    ...
    int searchIndex(E info)
    {
        for(int i = 0; i<adjList.length; i++)
        {
            if(adjList[i].getInfo().equals(info))
                return i;
        }
        return -1;
    }
...
class GraphNode<E> {
    ...
    public boolean equals(Object obj) {
        ...
        if(pom.info == null && this.info == null)
            return true;
        else if(pom.info == null || this.info == null)
            return false;
