...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    public int depth(BNode<E> t)
    {
        return privateDepth(t, root);
    }
    private int privateDepth(BNode<E> t, BNode<E> start)
    {
        if(start.info.compareTo(t.info) == 0)
            return 0;
        else if(t.info.compareTo(start.info) == -1)
            return 1 + privateDepth(t, start.left);
        else
            return 1+privateDepth(t, start.right);
    }
    /**
...
public class BinarnoDrvo {
    public static void main(String[] args) throws IOException {
        ...
        BNode<Integer> node = tree.find(element);
        System.out.println(tree.height(node));
        System.out.println(tree.depth(node));
    }
}
