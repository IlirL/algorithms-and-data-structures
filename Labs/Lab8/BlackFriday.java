...
public class BlackFriday {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Heap<Buyer> priorityQueue = new Heap<Buyer>(n);
        ...
        int nOfBuyersSameTime = 1;
        int maxBuyersSameTime = 0;
        for(int i = 0; i<n-1;i++)
        {
            Buyer maxTimeBuyer = priorityQueue.removeMax();
            for(int j = 0; j<priorityQueue.getSize(); j++)
            {
                if(maxTimeBuyer.buyingSameTime(priorityQueue.getAt(j)))
                    nOfBuyersSameTime++;
            }
            maxBuyersSameTime = Math.max(maxBuyersSameTime, nOfBuyersSameTime);
            nOfBuyersSameTime = 1;
        }

        System.out.println(maxBuyersSameTime);
    }
}
...
class Buyer implements Comparable<Buyer>
{
    ...
    public Buyer(String entryTime, int totalTime)
    {
        String[] parts = entryTime.split(":");
        this.entryMinutes = Integer.parseInt(parts[1]) + Integer.parseInt(parts[0])*60;
        this.totalTime = totalTime;

        if(entryMinutes + totalTime > 1439)
            this.totalTime -= (entryMinutes + totalTime) - 1439;
    }

    @Override
    public int compareTo(Buyer other)
    {
        return ((entryMinutes + totalTime)  - (other.entryMinutes + other.totalTime))*(-1);
    }

    public boolean buyingSameTime(Buyer other)
    {
        if((entryMinutes + totalTime) >= (other.entryMinutes))
            return true;
        return false;
    }
