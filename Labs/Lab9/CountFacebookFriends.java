...
public class CountFacebookFriends {
    public static void main(String[] args) {
        ...
        Graph<Integer> graph = new Graph<Integer>(numberOfUsers);
        //We actually dont need the username and surname, we just need the ordinal number
        for(int i = 0; i<numberOfUsers; i++)
        {
            int numberOfFriends = Integer.parseInt(s.nextLine());
            for(int j = 0; j<numberOfFriends; j++) {
//                String line = s.nextLine();
                int friend = Integer.parseInt(s.nextLine().split(" ")[0]);
//                System.out.println("friend = " + friend);
                graph.addEdge(i, friend);
            }
        }
//        System.out.println(graph.toString());
        int friend1 = Integer.parseInt(s.nextLine());
        int friend2 = Integer.parseInt(s.nextLine());

        System.out.println(graph.bfs(friend1, friend2));
    }
}
...
class Graph<E> {
    ...
    Integer bfs(int node, int friend){
//		System.out.println("Node = " + node + "  friend = " + friend);
        boolean visited[] = new boolean[num_nodes];
        for (int i = 0; i < num_nodes; i++)
            visited[i] = false;
        visited[node] = true;
//		System.out.println(node+": " + nodes[node]);
        ...
        int pom;
//		int height = 0;
        Queue<Integer> heights = new LinkedQueue<Integer>();
        heights.enqueue(0);
        while(!q.isEmpty()){
            pom = q.dequeue();
//			height++;
...
//						System.out.println(i+": " + nodes[i]);
                        q.enqueue(i);
                        heights.enqueue(heights.peek() + 1);
                        if(i == friend)
                        {
