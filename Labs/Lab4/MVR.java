...
public class MVR {
    public static void otvoriShalter(ArrayQueue<Gragjanin> redicaLkarti,
        ...
        {
            Gragjanin temp_gragjan = redicaLkarti.dequeue();
            if(temp_gragjan.pasos == 1)
                redicaPasos.enqueue(temp_gragjan);
            else if(temp_gragjan.vozacka == 1)
                redicaVozacka.enqueue(temp_gragjan);
            else
                System.out.println(temp_gragjan.imePrezime);
        }

        while(!redicaPasos.isEmpty())
        {
            Gragjanin temp_gragjan = redicaPasos.dequeue();

             if(temp_gragjan.vozacka == 1)
                redicaVozacka.enqueue(temp_gragjan);
            else
                System.out.println(temp_gragjan.imePrezime);
        }
        ...
        {
            Gragjanin temp_gragjan = redicaVozacka.dequeue();
                System.out.println(temp_gragjan.imePrezime);
        }
