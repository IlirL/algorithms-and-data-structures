...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> firstCard_firstFriend = firstFriendCards.getFirst();
        SLLNode<Card> bestCard = firstCard_firstFriend;

        SLLNode<Card> middle_card = secondFriendCards.getFirst();
        int size = secondFriendCards.length();
        size/=2;
        while(firstCard_firstFriend != null)
        {
            SLLNode<Card> current = firstCard_firstFriend;
            if(current.element.damage() > bestCard.element.damage())
            {
                bestCard = current;
            }
            firstCard_firstFriend = firstCard_firstFriend.succ;
        }
        while(size>1)
        {
            middle_card = middle_card.succ;
            size--;
        }
        firstFriendCards.delete(bestCard);
        secondFriendCards.insertAfter(bestCard.element, middle_card);
    }
