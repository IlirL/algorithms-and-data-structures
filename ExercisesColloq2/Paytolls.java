...
public class Paytolls {
    public static void main(String[] args) throws IOException {
        ...
        Graph<Integer> graph = new Graph<Integer>(n, nodes);
        int skopje, berlin;
        String[] ids = br.readLine().split("\\s+");
        skopje = Integer.parseInt(ids[0]);
        berlin = Integer.parseInt(ids[1]);
        skopje--; berlin--; //becuase we have nodes starting from 0 while in task is given they start at 1
        ...
        for(int i = 0; i<m; i++)
        {
            ...
            int tezina = Integer.parseInt(parts[2]);
            graph.addEdge(--node1, --node2, tezina);

        }

        System.out.println((int)graph.dijkstra(skopje)[berlin]);
    }
}
