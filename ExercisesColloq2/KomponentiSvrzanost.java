...
class Graph<E> {
    ...
    int [] najdikomponenti(int node){

        ArrayList<Integer> komponenti = new ArrayList<Integer>();
        //vasiot kod tuka
        boolean visited[] = new boolean[num_nodes];
        for (int i = 0; i < num_nodes; i++)
            visited[i] = false;
        visited[node] = true;
//        System.out.println(adjList[node].getIndex());
        komponenti.add(adjList[node].getIndex());
...
//                    System.out.println(tmp.getIndex());
                    komponenti.add(tmp.getIndex());
                    q.enqueue(tmp.getIndex());
                }
            }
        }
     int array[] = new int[komponenti.size()];
        for(int i = 0; i<komponenti.size(); i++)
        {
            array[i] = komponenti.get(i);
        }
        return array;
...
public class KomponentiSvrzanost {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //vasiot kod tuka
        komponenti = Arrays.stream(komponenti).sorted().toArray();
